...
function renderTeamSlots() { 
    ...
    el.innerHTML = filled.map(({ slot, i }, displayIndex) => { 
        ...
        const scoreBadge = assassinScore !== null
            ...
            : '';
        const head = `<div class="slotHead">${spriteImg(p)}<div><div class="slotNum">Team ${displayIndex + 1}/${TEAM_SIZE} · #${String(p.id).padStart(4, '0')}${bst ? ` · <span title="Base Stat Total" style="color:${bstColor}; font-weight:900;">BST ${bst}</span>` : ''}</div><div class="slotName">${p.name.replace(/-/g, ' ')}</div><div class="slotTypes">${p.types.map(t => tb(t)).join('')}</div><div class="cardActions"><button class="mapActionBtn" type="button" data-world-map-pokemon="${encodeURIComponent(p.name.replace(/-/g, ' '))}">🗺️ Open area on map</button></div></div><div class="slotActions" style="margin-left:auto;">${scoreBadge}<button class="calcToggle ${slot.calc ? 'on' : ''}" type="button" data-calc="${i}">${slot.calc ? 'In calculate' : 'Add to calc'}</button><button class="clearSlot" type="button" data-clear="${i}" title="Clear slot">×</button></div></div>`; 
...
document.getElementById('teamSlots').addEventListener('click', e => { 
    ...
    if (mapBtn) { if (typeof openWorldMap === 'function') openWorldMap({ pokemon: decodeURIComponent(mapBtn.dataset.worldMapPokemon) }); return; }
