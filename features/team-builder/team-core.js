...
const TEAM_NATURE_EFFECTS = {
    Lonely: ['ATK', 'DEF'], Brave: ['ATK', 'SPD'], Adamant: ['ATK', 'SPATK'], Naughty: ['ATK', 'SPDEF'],
    Bold: ['DEF', 'ATK'], Relaxed: ['DEF', 'SPD'], Impish: ['DEF', 'SPATK'], Lax: ['DEF', 'SPDEF'],
    Timid: ['SPD', 'ATK'], Hasty: ['SPD', 'DEF'], Jolly: ['SPD', 'SPATK'], Naive: ['SPD', 'SPDEF'],
    Modest: ['SPATK', 'ATK'], Mild: ['SPATK', 'DEF'], Quiet: ['SPATK', 'SPD'], Rash: ['SPATK', 'SPDEF'],
    Calm: ['SPDEF', 'ATK'], Gentle: ['SPDEF', 'DEF'], Sassy: ['SPDEF', 'SPD'], Careful: ['SPDEF', 'SPATK']
};
...
window.getEffectivePokemonName = function(baseName, itemName) {
    ...
    if (itemData && itemData.mechanics && 
       (itemData.mechanics.effectType === 'mega_evolution' || itemData.mechanics.effectType === 'primal_reversion')) {
        ...
        if (String(baseName).toLowerCase() === String(itemData.mechanics.target).toLowerCase()) {
            return `${baseName}${itemData.mechanics.suffix}`.toLowerCase();
        }
    }
    return baseName;
};
