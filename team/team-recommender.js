// --- team-recommender.js : Pro-Tier Move & Item Optimizer V3 ---

window.showMoveRecommendations = function() {
    const selected = typeof calcTeam === 'function' ? calcTeam() : [];
    if (!selected.length) {
        alert('Add some Pokémon to Battle Calculate first so I can suggest moves & items!');
        return;
    }
    ...
    selected.forEach(x => {
        ...
        if (isTank) {
            ...
            if (x.p.types.includes("Poison")) recItems.push("Black Sludge");
        } else if (isPhysical) {
            recItems.push("Choice Band", "Choice Scarf", "Life Orb", "Focus Sash");
        } else if (isSpecial) {
            recItems.push("Choice Specs", "Choice Scarf", "Life Orb", "Focus Sash");
        } else {
            recItems.push("Life Orb", "Expert Belt", "Focus Sash");
        }
        
        let itemsHtml = `<div style="margin-top:10px; padding-top:10px; border-top:1px solid rgba(255,255,255,0.1);">
            <strong style="color:#ffd43b; font-size:13px;">🎒 Recommended Items:</strong>
            <div style="display:flex; gap:8px; margin-top:5px; flex-wrap:wrap;">
                ${recItems.map(item => `<span style="background:rgba(255, 212, 59, 0.2); color:#ffd43b; font-size:11px; padding:3px 8px; border-radius:12px; border:1px solid #ffd43b;">${item}</span>`).join('')}
            </div>
        </div>`;
        // ---------------------------------------
        ...
        if (moveList.length) {
            moveList.forEach(mName => {
                ...
                if (mInfo.cat === 'status') {
                    ...
                    if (isTank && score === 0) score += 30; 
                } else {
                    if (x.p.types.includes(mInfo.type)) { score += 100; reasons.push('💥 STAB'); }
                    if (PRO_MOVES.UTILITY.includes(cleanName)) { score += 180; reasons.push('🔄 Pro Utility'); } 
